...
public:
	QueryResult(const string& s, shared_ptr<set<unsigned>>p, shared_ptr<StrVec>f)
		:sought_word(s), lineNo(p), file(f) {}
	friend ostream& print(ostream&, const QueryResult&);
};
TextQuery::TextQuery(ifstream& input) :input_file(new StrVec)
{
	string line, word;
